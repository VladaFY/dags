...
from DDS_dag.clean_prepare_df import drop_duplicates, check_table
...
with DAG(dag_id='source_to_dds_dag',
    ...
    check_table_task_employees = PythonOperator(
        task_id='check_table', python_callable=check_table, op_kwargs={'table_dds': 'vlada_test.test_employees_dds'})
    
    transfer_data_employees = PythonOperator(
        task_id='transfer_data', python_callable=drop_duplicates, op_kwargs={'table': 'vlada_test.test_employees', 'target_table': 'vlada_test.test_employees_dds', 'primary_key': 'id'})
    
    check_table_task_salary = PythonOperator(
        task_id='check_table', python_callable=check_table, op_kwargs={'table_dds': 'vlada_test.test_salary_dds'})
    
    transfer_data_salary = PythonOperator(
        task_id='transfer_data', python_callable=drop_duplicates, op_kwargs={'table': 'vlada_test.test_salary', 'target_table': 'vlada_test.test_salary_dds', 'primary_key': ['id', 'dt', 
        'salary_Type']})
     
    check_table_task_emails = PythonOperator(
        task_id='check_table', python_callable=check_table, op_kwargs={'table_dds': 'vlada_test.test_emails_dds'})
    
    transfer_data_emails = PythonOperator(
        task_id='transfer_data', python_callable=drop_duplicates, op_kwargs={'table': 'vlada_test.test_emails', 'target_table': 'vlada_test.test_emails_dds', 'primary_key': 'id'})
